...
LINUX_PACKAGE_DIR = $(BASE_DIR)/packages/linux
...
LINUX_MAKE_FLAGS = \
	...
	ARCH=$(KERNEL_ARCH) \
	INSTALL_MOD_PATH=$(LINUX_PACKAGE_DIR) \
...
define LINUX_INSTALL_KERNEL_IMAGE_TO_TARGET
	install -m 0644 -D $(LINUX_IMAGE_PATH) $(LINUX_PACKAGE_DIR)/boot/$(LINUX_IMAGE_NAME)
...
define LINUX_INSTALL_TARGET_DIR
	mkdir -p $(TARGET_DIR)/lib
	if [ -d $(LINUX_PACKAGE_DIR)/lib/ ] ; then	\
		cp -avr $(LINUX_PACKAGE_DIR)/lib/* $(TARGET_DIR)/lib/;	\
	fi
endef
...
define LINUX_INSTALL_TARGET_CMDS
	...
	@if grep -q "CONFIG_MODULES=y" $(@D)/.config; then 	\
		...
		rm -f $(LINUX_PACKAGE_DIR)/lib/modules/$(LINUX_VERSION_PROBED)/build ;		\
		rm -f $(LINUX_PACKAGE_DIR)/lib/modules/$(LINUX_VERSION_PROBED)/source ;	\
		rm -f $(LINUX_PACKAGE_DIR)/lib/modules/modules_kernel;				\
		ln -s $(LINUX_VERSION_PROBED)/kernel $(LINUX_PACKAGE_DIR)/lib/modules/modules_kernel ; \
	fi
	$(LINUX_INSTALL_HOST_TOOLS)
	$(LINUX_INSTALL_TARGET_DIR)
endef
