...
TOOLCHAIN_PACKAGE_DIR = $(BASE_DIR)/packages/toolchain

# Install default nsswitch.conf file if the skeleton doesn't provide it
ifeq ($(BR2_TOOLCHAIN_USES_GLIBC),y)
define GLIBC_COPY_NSSWITCH_FILE
	$(Q)if [ ! -f "$(TOOLCHAIN_PACKAGE_DIR)/etc/nsswitch.conf" ]; then \
		$(INSTALL) -D -m 0644 package/glibc/nsswitch.conf $(TOOLCHAIN_PACKAGE_DIR)/etc/nsswitch.conf ; \
	fi
endef
...
define COPY_GCONV_LIBS
	...
	if [ -z "$(GCONV_LIBS)" ]; then \
		$(INSTALL) -m 0644 -D $(STAGING_DIR)/usr/lib/$${d}/gconv/gconv-modules \
				      $(TOOLCHAIN_PACKAGE_DIR)/usr/lib/gconv/gconv-modules && \
		$(INSTALL) -m 0644 $(STAGING_DIR)/usr/lib/$${d}/gconv/*.so \
				   $(TOOLCHAIN_PACKAGE_DIR)/usr/lib/gconv \
		|| exit 1; \
	else \
		for l in $(GCONV_LIBS); do \
			$(INSTALL) -m 0644 -D $(STAGING_DIR)/usr/lib/$${d}/gconv/$${l}.so \
					      $(TOOLCHAIN_PACKAGE_DIR)/usr/lib/gconv/$${l}.so \
			|| exit 1; \
			...
			while read lib; do \
				 $(INSTALL) -m 0644 -D $(STAGING_DIR)/usr/lib/$${d}/gconv/$${lib} \
						       $(TOOLCHAIN_PACKAGE_DIR)/usr/lib/gconv/$${lib} \
				 || exit 1; \
			done; \
		done; \
