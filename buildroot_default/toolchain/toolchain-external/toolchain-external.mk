...
#  $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR). Obviously, things such as the C library, the dynamic
...
TOOLCHAIN_EXTERNAL_PACKAGE_DIR = $(BASE_DIR)/packages/toolchain
...
define TOOLCHAIN_EXTERNAL_LINARO_ARMHF_SYMLINK
	mkdir -p $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/
	mkdir -p $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/
	ln -snf . $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/arm-linux-gnueabihf
	ln -snf . $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/arm-linux-gnueabihf
endef

define TOOLCHAIN_EXTERNAL_LINARO_ARMEBHF_SYMLINK
	mkdir -p $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/
	mkdir -p $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/
	ln -snf . $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/armeb-linux-gnueabihf
	ln -snf . $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/armeb-linux-gnueabihf
endef

define TOOLCHAIN_EXTERNAL_LINARO_AARCH64_SYMLINK
	mkdir -p $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/
	mkdir -p $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/
	ln -snf . $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/aarch64-linux-gnu
	ln -snf . $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/aarch64-linux-gnu
endef
...
define TOOLCHAIN_EXTERNAL_MUSL_LD_LINK
	mkdir -p $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/
	ln -sf libc.so $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/ld-musl-$(MUSL_ARCH).so.1
endef
...
# the $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR) and copy the whole sysroot (libraries and headers)
# to $(STAGING_DIR).
#
	...
	if test "$(BR2_TOOLCHAIN_EXTERNAL_GDB_SERVER_COPY)" = "y"; then \
		...
		for d in $${ARCH_SYSROOT_DIR}/usr \
			...
			if test -f $${d}/bin/gdbserver ; then \
				install -m 0755 -D $${d}/bin/gdbserver $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/bin/gdbserver ; \
				gdbserver_found=1 ; \
				break ; \
			fi ; \
		done ; \
...
define TOOLCHAIN_EXTERNAL_INSTALL_BFIN_FLAT
	...
	if [ -f $${FLAT_LIBC_A_LOCATION} -a ! -h $${FLAT_LIBC_A_LOCATION} ] ; then \
	        $(INSTALL) -D $${FLAT_LIBC_A_LOCATION} $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/lib1.so; \
	fi
endef
endif
...
define TOOLCHAIN_EXTERNAL_FIXUP_UCLIBCNG_LDSO
	$(Q)if test -e $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/ld-uClibc.so.1; then \
		ln -sf ld-uClibc.so.1 $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/ld-uClibc.so.0 ; \
	fi
	$(Q)if test -e $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/ld64-uClibc.so.1; then \
		ln -sf ld64-uClibc.so.1 $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/ld64-uClibc.so.0 ; \
	fi
endef
...
define TOOLCHAIN_EXTERNAL_INSTALL_TARGET_DIR
	#mkdir -p $(TARGET_DIR)/etc
	#cp -avr $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/etc/* 	$(TARGET_DIR)/etc/

	mkdir -p $(TARGET_DIR)/lib
	cp -avr $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/lib/* 	$(TARGET_DIR)/lib/

	mkdir -p $(TARGET_DIR)/usr/lib
	cp -av $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/libstdc++.so.6		$(TARGET_DIR)/usr/lib/
	cp -av $(TOOLCHAIN_EXTERNAL_PACKAGE_DIR)/usr/lib/libstdc++.so.6.?.??	$(TARGET_DIR)/usr/lib/
endef
...
define TOOLCHAIN_EXTERNAL_INSTALL_TARGET_CMDS
	...
	$(TOOLCHAIN_EXTERNAL_INSTALL_TARGET_DIR)
endef
