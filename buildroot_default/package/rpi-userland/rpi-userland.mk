...
RPI_USERLAND_PACKAGE_DIR = $(BASE_DIR)/packages/rpi_userland
RPI_USERLAND_PROVIDES = libegl libgles libopenmax libopenvg

ifeq ($(BR2_PACKAGE_RPI_USERLAND_START_VCFILED),y)
define RPI_USERLAND_INSTALL_INIT_SYSV
	mkdir -p $(RPI_USERLAND_PACKAGE_DIR)/etc/init.d/
	$(INSTALL) -m 0755 -D package/rpi-userland/S94vcfiled \
		$(RPI_USERLAND_PACKAGE_DIR)/etc/init.d/S94vcfiled
endef
define RPI_USERLAND_INSTALL_INIT_SYSTEMD
	mkdir -p $(RPI_USERLAND_PACKAGE_DIR)/usr/lib/systemd/system/
	$(INSTALL) -D -m 644 package/rpi-userland/vcfiled.service \
		$(RPI_USERLAND_PACKAGE_DIR)/usr/lib/systemd/system/vcfiled.service
	mkdir -p $(RPI_USERLAND_PACKAGE_DIR)/etc/systemd/system/multi-user.target.wants/
	ln -sf ../../../../usr/lib/systemd/system/vcfiled.service \
		$(RPI_USERLAND_PACKAGE_DIR)/etc/systemd/system/multi-user.target.wants/vcfiled.service
endef
endif
...
define RPI_USERLAND_EXTRA_LIBS_TARGET
	mkdir -p $(RPI_USERLAND_PACKAGE_DIR)/usr/lib/
	$(INSTALL) -m 0644 -D \
		$(@D)/build/lib/libilclient.so \
		$(RPI_USERLAND_PACKAGE_DIR)/usr/lib/libilcient.so
endef
...
define RPI_USERLAND_POST_TARGET_CLEANUP
	rm -f $(RPI_USERLAND_PACKAGE_DIR)/etc/init.d/vcfiled
	rm -f $(RPI_USERLAND_PACKAGE_DIR)/usr/share/install/vcfiled
	#rmdir --ignore-fail-on-non-empty $(RPI_USERLAND_PACKAGE_DIR)/usr/share/install
	rm -Rf $(RPI_USERLAND_PACKAGE_DIR)/usr/src
endef
