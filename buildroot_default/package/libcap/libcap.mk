...
LIBCAP_PACKAGE_DIR = $(BASE_DIR)/packages/libcap
...
define LIBCAP_INSTALL_TOOLS_CMDS
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(@D)/progs \
		...
		DESTDIR=$(LIBCAP_PACKAGE_DIR) $(LIBCAP_MAKE_FLAGS) install
endef
endif
...
define LIBCAP_INSTALL_TARGET_DIR
	mkdir -p $(TARGET_DIR)/usr/lib
	cp -av $(LIBCAP_PACKAGE_DIR)/usr/lib/libcap.so		$(TARGET_DIR)/usr/lib
	cp -av $(LIBCAP_PACKAGE_DIR)/usr/lib/libcap.so.2	$(TARGET_DIR)/usr/lib
	cp -av $(LIBCAP_PACKAGE_DIR)/usr/lib/libcap.so.2.24 	$(TARGET_DIR)/usr/lib
endef

define LIBCAP_INSTALL_TARGET_CMDS
	$(TARGET_MAKE_ENV) $(MAKE) -C $(@D)/libcap $(LIBCAP_MAKE_FLAGS) \
		DESTDIR=$(LIBCAP_PACKAGE_DIR) prefix=/usr lib=lib $(LIBCAP_MAKE_INSTALL_TARGET)
	$(LIBCAP_INSTALL_TOOLS_CMDS)
	$(LIBCAP_INSTALL_TARGET_DIR)
endef
