...
BUSYBOX_PACKAGE_DIR = $(BASE_DIR)/packages/busybox/
...
BUSYBOX_MAKE_OPTS = \
	CC="$(TARGET_CC)" \
	ARCH=$(KERNEL_ARCH) \
	PREFIX="$(BUSYBOX_PACKAGE_DIR)" \
	EXTRA_LDFLAGS="$(BUSYBOX_LDFLAGS)" \
	CROSS_COMPILE="$(TARGET_CROSS)" \
	CONFIG_PREFIX="$(BUSYBOX_PACKAGE_DIR)" \
	SKIP_STRIP=y
...
ifeq ($(BR2_TARGET_ROOTFS_CHROOT),y)
	BUSYBOX_CONIG_FILE = package/busybox/busyboxbsp.config
endif
...
define BUSYBOX_INSTALL_MDEV_SCRIPT
	$(INSTALL) -D -m 0755 package/busybox/S10mdev \
		$(BUSYBOX_PACKAGE_DIR)/etc/init.d/S10mdev
endef
define BUSYBOX_INSTALL_MDEV_CONF
	$(INSTALL) -D -m 0644 package/busybox/mdev.conf \
		$(BUSYBOX_PACKAGE_DIR)/etc/mdev.conf
endef
...
define BUSYBOX_INSTALL_UDHCPC_SCRIPT
	if grep -q CONFIG_UDHCPC=y $(@D)/.config; then \
		$(INSTALL) -m 0755 -D package/busybox/udhcpc.script \
			$(BUSYBOX_PACKAGE_DIR)/usr/share/udhcpc/default.script; \
		$(INSTALL) -m 0755 -d \
			$(BUSYBOX_PACKAGE_DIR)/usr/share/udhcpc/default.script.d; \
	fi
endef
...
define BUSYBOX_INSTALL_LOGGING_SCRIPT
	if grep -q CONFIG_SYSLOGD=y $(@D)/.config; then \
		$(INSTALL) -m 0755 -D package/busybox/S01logging \
			$(BUSYBOX_PACKAGE_DIR)/etc/init.d/S01logging; \
		rm -f $(BUSYBOX_PACKAGE_DIR)/etc/init.d/S01logging; \
	fi
endef

ifeq ($(BR2_INIT_BUSYBOX),y)
define BUSYBOX_INSTALL_INITTAB
	$(INSTALL) -D -m 0644 package/busybox/inittab $(BUSYBOX_PACKAGE_DIR)/etc/inittab
endef
endif
...
define BUSYBOX_INSTALL_WATCHDOG_SCRIPT
	$(INSTALL) -D -m 0755 package/busybox/S15watchdog \
		$(BUSYBOX_PACKAGE_DIR)/etc/init.d/S15watchdog
...
define BUSYBOX_INSTALL_TELNET_SCRIPT
	if grep -q CONFIG_FEATURE_TELNETD_STANDALONE=y $(@D)/.config; then \
		$(INSTALL) -m 0755 -D package/busybox/S50telnet \
			$(BUSYBOX_PACKAGE_DIR)/etc/init.d/S50telnet ; \
	fi
endef
...
define BUSYBOX_INSTALL_TARGET_DIR
	mkdir -p $(TARGET_DIR)/
	cp -avr $(BUSYBOX_PACKAGE_DIR)/* $(TARGET_DIR)/
endef
