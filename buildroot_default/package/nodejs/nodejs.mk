...
NODEJS_PACKAGE_DIR = $(BASE_DIR)/packages/nodejs
...
NPM = $(TARGET_CONFIGURE_OPTS) \
	...
	npm_config_prefix=$(NODEJS_PACKAGE_DIR)/usr \
...
define NODEJS_INSTALL_TARGET_DIR
	cp -avr $(NODEJS_PACKAGE_DIR)/usr/bin/* $(TARGET_DIR)/usr/bin/
	cp -avr $(NODEJS_PACKAGE_DIR)/usr/lib/* $(TARGET_DIR)/usr/lib/
endef

define NODEJS_INSTALL_TARGET_CMDS
	$(TARGET_MAKE_ENV) PYTHON=$(HOST_DIR)/usr/bin/python2 \
		$(MAKE) -C $(@D) install \
		DESTDIR=$(NODEJS_PACKAGE_DIR) \
		$(TARGET_CONFIGURE_OPTS) \
		LD="$(TARGET_CXX)"
	$(NODEJS_INSTALL_MODULES)
	$(NODEJS_INSTALL_TARGET_DIR)
endef
